...
function mostrarResultados(results) {
  ...
  for (const resultados of results) {
    // resultados es la variable que creamos y results es el parametro de la funcion (mostrarResultados) que me trae la data
    // console.log(resultados);
    const titleEl = template.content.querySelector(".result-item-title");
    titleEl.textContent = resultados.title;

    const imgAltEl = template.content.querySelector(".result-item-img")
    imgAltEl.alt = resultados.title
    ...
    const linkmlEl = template.content.querySelector(".result-item-link-ml")
    linkmlEl.href = resultados.permalink
